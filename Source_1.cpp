...
struct TSnake
{
    ...
    bool mouth = false;
    int step = 2;
    TSnake() { dir = RIGHT; seg[0] = { 5, 5 }; lenght = 1; }
...
void ShowAplle0()
{
    ...
    for (int j = -1; j < 2; j += 2)
        ...
        {
            x = j * 1.75 * i / M_PI * sin(i) * 0.5 * (1 - gap * 2) + 0.5;
            y = 1.5 * i / M_PI * cos(i) * 0.5 * (1 - gap * 2) + 0.75;
            glVertex2f(x, y);
        }
    glEnd();
}
...
{
    int nseg = (map[j][i].snk.nseg);
    if (cell.apl) ShowAplle();
    if (cell.snk.bd)
    {
        ShowBody(i, j);
        //if (i == 1 && snake.seg[nseg].dir == 0) ShowBody(width - 1, j);
   }
...
void GameOver()
{
    glTranslatef(5, height / 2, 0);
    glScalef(1, -1, 1);
    snprintf(str, sizeof str, "%s", "GAME OVER!");
    print_string(0, 0, str, 0, 1, 0);
    Sleep(3000);
}
